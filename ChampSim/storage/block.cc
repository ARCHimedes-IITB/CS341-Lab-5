 ...
 * 
 * @took_miss - whether this eviction was actually caused due
 * to a miss. For now this is always true, but one can imagine
 * cases when it may not be, e.g. we prefetched some data
 * which evicted this line.
 * 
 * Further, if we just took a miss which caused this fill, we
 * should look at what policy we were following for this
 * cacheline and update the global counter accordingly -- this
 * is far from ideal, but I don't see any other way to do a 
 * lru v/s bypass duel.
 */
void PREDICTOR::update_evicted(int index, bool reuse, bool took_miss=true)
{
  // If we are using a global overriding policy, no need to do anything
  if (global_override_policy != DYNAMIC)
    return;
  ...
  if (took_miss) {
    // We just took a miss
    if ((pred_entries[index].counter >> 5) & 1) {
      // We were bypassing to L2 - improve affinity
      // towards LRU by decrementing the global counter
      default_policy_counter--;
    } else {
      // We were using LRU here - improve affinity
      // toward bypassing by incrementing the global counter
      default_policy_counter++;
    }
  }
