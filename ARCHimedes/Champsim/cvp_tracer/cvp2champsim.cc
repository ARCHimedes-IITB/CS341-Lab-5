...
// defines for the paths for the various decompression programs and Apple/Linux
// differences
...
struct trace {
  ...
  uint8_t access_size,
      ...
      num_input_regs, num_output_regs, input_reg_names[256],
  ...
  // read a single record from the trace file, return true on success, false on
  // EOF
  ...
  bool read(FILE *f) {
    ...
    case uncondIndirectBranchInstClass:
      ...
      if (taken) {
        ...
        // this had better not be an unconditional branch (frickin ARM with its
        // predicated branches)
...
bool is_branch(InstClass t) {
  return (t == uncondIndirectBranchInstClass ||
          t == uncondDirectBranchInstClass || t == condBranchInstClass);
}
...
// this string will contain the trace file name, or "-" if we want to read from
// standard input
...
UINT64 transform(UINT64 a) {
  ...
  if (code_pages.find(page) != code_pages.end()) {
    ...
    if (new_page == 0) {
      ...
      new_page = bump_page;
      for (;;) {
        if (code_pages.find(new_page) != code_pages.end() ||
...
int main(int argc, char **argv) {
  ...
  for (;;) {
    ...
    if (is_branch(t.type)) {
      ...
      // if this is a conditional branch then it's direct and we're done
      ...
      if (t.type == condBranchInstClass) {
        ...
        // on ARM, calls link the return address in register X30. let's see if
        // this instruction is doing that; if so, it's a call or wants us to
        // believe it is
        ...
        // on ARM, returns are an indirect jump to X30. let's see if we're doing
        // this
      ...
      case OPTYPE_JMP_DIRECT_COND:
        ...
        ct.destination_registers[0] = REG_IP;
        // turns out pin records conditional direct branches as also reading IP.
        // whatever.
  ...
  for (int i = 2; i < (int)lim; i++) {
    if (counts[i])
      fprintf(stderr, "%s %lld %f%%\n", branch_names[i], counts[i],
              100 * counts[i] / (double)n);
  }
